...
bool Moderate::executeBid(Game *g) {
    string bid;
    do {
...
int Moderate::Bid(Game* g, Player* p, int currentRoundBidderIndex, int auctionRoundPlayersRemaining, int oneRemainingPlayer) {
    ...
    while (auctionRoundPlayersRemaining > oneRemainingPlayer) {
        ...
        //The last bidder to have bid for the powerplant is stored just in case
...
void Moderate::buyCities(Map *map, Player* p, int gamePhaseNumber) {
    //DECLARE VARIABLES
    ...
    while (true) {
        ...
        while (!validEntry) {
            ...
            if(p->getNumOfCities() == 6) {
                std::cout << "Moderate player does not want to reach step 2, PASSING\n\n";
                return;
            }
        ...
        if (totalCost <= p->getElektros() && houseAvailableAtLocation) {
            ...
            p->addHouse(nameOfNewHouseLocation);
