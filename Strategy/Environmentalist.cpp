...
void Environmentalist::executeAuction(Game* g, Player* p) {
    ...
    do {
        ...
        cout << g->pMarket << endl;
        for(const Powerplant& plant : g->pMarket.getCurrentMarket()) {
            i++;
            if(plant.getResourceType() == "Ecological") {
                bid = std::to_string(i);
                break;
            }
        }
...
bool Environmentalist::executeBid(Game *g) {
    string bid;
    do {
...
void Environmentalist::buyCities(Map *map, Player* p, int gamePhaseNumber) {
    //DECLARE VARIABLES
